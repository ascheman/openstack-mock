...
A GitHub workflow builds a multi-architecture container image for this project and publishes to it to GitHub Container Registry (GHCR) as `ghcr.io/uhurutec/openstack-mock`

* The build tags images on the main branch by `latest` and the short commit SHA.
* On other branches the build tags the image with a branch-based name and the short commit SHA.

NOTE: We currently only provide private images on the registry repository.

[IMPORTANT]
====
To allow the GitHub Action to publish to GHCR using GITHUB_TOKEN, ensure:

* Repository settings: Actions > General > Workflow permissions = Read and write permissions.
* Organization/Repository package settings: Allow GitHub Actions to create and update packages in GHCR.
* Forks: Workflows in forks will push to their own GHCR namespace (ghcr.io/<fork-owner>/openstack-mock) due to dynamic owner selection.
====
